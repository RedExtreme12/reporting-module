...
class TableData:
    ...
    def _get_random_model_name(self):
    ...
    def _get_random_count_auto(self, limit=1000):
        return random.randrange(limit)

    def _get_random_price(self, low_price: float, high_price: float):
    ...
    def get_random_data(self, number_of_records):
        ...
        for i in range(number_of_records):
            random_data.add(
                (
                    self._get_random_model_name(),
                    self._get_random_date().strftime("%d.%m.%Y"),
                    self._get_random_count_auto(CAR_COUNT_LIMIT),
                    self._get_random_price(CAR_PRICE_LOW_BOUND, CAR_PRICE_HIGH_BOUND)
                )
            )
